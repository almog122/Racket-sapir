...
; Purpose: Consistent replacement of all FREE occurrences of
;            the substitution variables in 
;          'expr' by the values of the substitution, respectively.
;          'expr' is a Scheme expression.
;Type: Client view:
;     [Substitution*Scheme-expression union Evaluator-value ->
;               Scheme-expression union Evaluator-value]
;      Supplier view: [LIST*(LIST union Symbol) -> LIST union Symbol]
;Example: (substitute 
;            (make-sub '(x y z) '(3 4 (lambda (x1)(+ x1 1))))
;            '(lambda (x2)(+ x2 y))) ) ==>  (lambda (x2)(+ x2 4))
; Pre-conditions:
;  (1) Substitution is not performed on 'define' or 'let' expressions
...
(define lookup-variable-value
  (lambda (var)
    (get-value-of-variable the-global-environment 
                           var
                           (lambda () (error 'lookup-variable-value "undefined varoable ~s" var)))
    ))
